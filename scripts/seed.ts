...
const supabaseUrl =
  process.env.EXPO_PUBLIC_SUPABASE_URL || 'https://fnnhciiooxgvffbdbwxr.supabase.co';
const supabaseServiceKey = process.env.SUPABASE_SERVICE_ROLE_KEY || '';

if (!supabaseServiceKey) {
  console.error('❌ Error: SUPABASE_SERVICE_ROLE_KEY is required for seeding.');
  console.error('Please add SUPABASE_SERVICE_ROLE_KEY to your .env or .env.local file.');
...
async function seed() {
  ...
  try {
    ...
    for (let i = 0; i < fakeUsers.length; i++) {
      ...
      console.log(`Creating user ${i + 1}/10: ${fakeUser.name} (${fakeUser.username})...`);

      try {
        // Check if user already exists
        const { data: existingUser } = await supabase.auth.admin.listUsers();
        const userExists = existingUser?.users?.some((u) => u.email === fakeUser.email);

        let userId: string;

        if (userExists) {
          console.log(`  ⚠️  User already exists, skipping auth creation...`);
          const user = existingUser.users.find((u) => u.email === fakeUser.email);
          if (!user) {
          ...
          if (authError) {
            ...
            const maybeCreated = checkUser?.users?.find((u) => u.email === fakeUser.email);

            if (maybeCreated) {
              console.log(`  ⚠️  User was created despite error, continuing...`);
          ...
          if (profileError) {
            console.error(`  ⚠️  Error upserting profile: ${profileError.message}`);
          } else {
    ...
    createdUsers.forEach((user, index) => {
      console.log(`${index + 1}. ${user.username} - Email: ${user.email}, Password: password123`);
    });
  } catch (error: any) {
